...
Public Class HSPI
  ...
  '
  Public CameraThread As Thread
  Public CameraEventThread As Thread
  Public MaintenanceThread As Thread
...
#Region "HSPI - Base Plugin API"
        ...
        Select Case dv_type
          Case "Welcome Person"
            ...
            If pdata IsNot Nothing Then
              Dim homes_id As String = pdata.GetNamed("homes_id")
              Dim dv_id As String = pdata.GetNamed("dv_id")

              Select Case dv_value
                Case 0
                  Dim NetatmoResponse As hspi_netatmo_api.NetatmoResponse = NetatmoAPI.SetPersonsAway(homes_id, dv_id)
                  If NetatmoResponse.status = "ok" Then
                    hs.SetDeviceValueByRef(CC.Ref, dv_value, True)
                    hs.SetDeviceString(CC.Ref, "Away", True)
                  Else
                    WriteMessage(String.Format("{0} action failed due to an unexpected error.", dv_type), MessageType.Warning)
                  End If

                Case 1
                  Dim NetatmoResponse As hspi_netatmo_api.NetatmoResponse = NetatmoAPI.SetPersonsHome(homes_id, dv_id)
                  If NetatmoResponse.status = "ok" Then
                    hs.SetDeviceValueByRef(CC.Ref, dv_value, True)
                    hs.SetDeviceString(CC.Ref, "Home", True)
                  Else
                    WriteMessage(String.Format("{0} action failed due to an unexpected error.", dv_type), MessageType.Warning)
                  End If
              End Select
          ...
          Case "Welcome State"
            ...
            Select Case dv_value
              Case -1
                ...
                WriteMessage("Welcome checking is enabled.", MessageType.Informational)
              ...
              Case -2
                ...
                WriteMessage("Welcome checking is disabled.", MessageType.Informational)
  ...
  Public Function InitIO(ByVal port As String) As String Implements HomeSeerAPI.IPlugInAPI.InitIO
    ...
    Try
      ...
      gSnapshotEventMaxWidth = GetSetting("Options", "SnapshotEventMaxWidth", gSnapshotMaxWidth)
      gEventVideoQuality = GetSetting("Options", "VideoQuality", gEventVideoQuality)

      gHSAppPath = hs.GetAppPath
      gImagePersonDirectory = FixPath(String.Format("{0}\html\images\hspi_ultranetatmowelcome3\persons", gHSAppPath))
      gImageCameraDirectory = FixPath(String.Format("{0}\html\images\hspi_ultranetatmowelcome3\cameras", gHSAppPath))
      gImageEventDirectory = FixPath(String.Format("{0}\html\images\hspi_ultranetatmowelcome3\events", gHSAppPath))

      '
      ' Ensure the image directories exist
      '
      If Directory.Exists(gImagePersonDirectory) = False Then
        Directory.CreateDirectory(gImagePersonDirectory)
      End If

      If Directory.Exists(gImageCameraDirectory) = False Then
        Directory.CreateDirectory(gImageCameraDirectory)
      End If

      If Directory.Exists(gImageEventDirectory) = False Then
        Directory.CreateDirectory(gImageEventDirectory)
      End If
      ...
      If bDBInitialized = True Then
        ...
        '
        ' Start the Welcome Thread
        '
        CameraThread = New Thread(New ThreadStart(AddressOf CheckCameraThread))
        CameraThread.Name = "CameraThread"
        CameraThread.Start()

        WriteMessage(String.Format("{0} Thread Started", CameraThread.Name), MessageType.Debug)

        '
        ' Start the Welcome Event Thread
        '
        CameraEventThread = New Thread(New ThreadStart(AddressOf CheckCameraEventThread))
        CameraEventThread.Name = "CameraEventThread"
        CameraEventThread.Start()

        WriteMessage(String.Format("{0} Thread Started", CameraEventThread.Name), MessageType.Debug)

        '
        ' Start the Snapshot File Maintenance Thread
        '
        MaintenanceThread = New Thread(New ThreadStart(AddressOf CheckMaintenanceThread))
        MaintenanceThread.Name = "MaintenanceThread"
        MaintenanceThread.Start()

        WriteMessage(String.Format("{0} Thread Started", MaintenanceThread.Name), MessageType.Debug)
  ...
  Public Sub ShutdownIO() Implements HomeSeerAPI.IPlugInAPI.ShutdownIO
    ...
    Try
      ...
      If CameraThread.IsAlive = True Then
        CameraThread.Abort()
      End If
        ...
        Case "Welcome Camera"
          ...
          If pdata IsNot Nothing Then
            ...
            SyncLock gNetatmoCamerasLock
              Dim camera_id As String = dv_addr.Replace("Camera-", "").Replace("-", ":")
              Dim Camera As hspi_netatmo_api.Camera = gNetatmoCameras.Find(Function(s) s.id = camera_id)

              If Not Camera Is Nothing Then
                Dim resource_url As String = String.Format("{0}/live/snapshot_720.jpg", Camera.camera_url)

                stb.AppendLine("<table cellspacing='0' width='100%'>")
                stb.AppendLine(" <tr>")
                stb.AppendFormat("  <th class='tableheader' style='text-align:left;' colspan='2'>{0}</th>", dv_type)
                stb.AppendLine(" </tr>")
                stb.AppendLine(" <tr>")
                stb.AppendFormat("  <td class='tablecell' style='width:150px'>{0}</td>", "Homes Id:")
                stb.AppendFormat("  <td class='tablecell'>{0}</td>", homes_id)
                stb.AppendLine(" </tr>")
                stb.AppendLine(" <tr>")
                stb.AppendFormat("  <td class='tablecell' style='width:150px'>{0}</td>", "Camera Id:")
                stb.AppendFormat("  <td class='tablecell'>{0}</td>", dv_id)
                stb.AppendLine(" </tr>")
                stb.AppendLine(" <tr>")
                stb.AppendFormat("  <td class='tablecell' style='width:150px'>{0}</td>", "Camera Snapshot URL:")
                stb.AppendFormat("  <td class='tablecell'>{0}</td>", resource_url)
                stb.AppendLine(" </tr>")
